...
> 版: v0.4（ケース 14 件。挑発と属性変化・ビリビリの判定のタイミングを旧作から変更）
...
2. **1 ターンの順番**: ② 先制コマンド（`defend` / `defenceBuff`。直前にビリビリの判定）→ ③ 敵の行動 → ④ 決着の判定（プレイヤー）→ ⑤ プレイヤーの行動（`attack` / `attackBuff`。直前にビリビリの判定）→ ⑥ 決着の判定（敵）→ ⑦ ターンを進める。
...
### 敵の値と行動（③）
- 敵の攻撃力 = `attack` × 属性の倍率 ＋ 挑発の上乗せ（属性の倍率とは別に持つ）。敵の攻撃倍率は 1 から始まる。
...
| `changeAttribute` | 敵の属性を「元の属性 ⇔ `changeAttribute`」で切り替え、属性の倍率を計算し直す。挑発の上乗せは残る |
...
### ビリビリ
6. プレイヤーの行動を実行する直前（先制コマンドは②、それ以外は⑤）に、残りが 1 以上なら残りを 1 減らして乱数を 1 回引く。`値 / 2^32 < 0.5` ならしびれて、その行動は行わない（敵の行動は行う）。
   - 敵がビリビリしたターン（③）でも、⑤の `attack` / `attackBuff` は判定を受ける。②の `defend` / `defenceBuff` はすでに終わっているので、そのターンは判定しない。
...
- 挑発の上乗せは属性変化で消えない（旧作は実装の都合で消えていた。直観に合わせて変更: 2026-09-26）。
- ビリビリの判定は行動の直前に行う（旧作はボタンを押した時点で判定していたため、ビリビリされたターンは効かなかった。敵が先に動くルールに合わせて変更: 2026-09-26）。
