...
> 版: v0.5（ケース 15 件。挑発と属性変化・ビリビリの判定のタイミング・敵の必殺技を旧作から変更）
...
| `deathblow` | プレイヤーの今の HP と同じダメージ（防御・属性・バフは関係ない）。必ず HP が 0 になって負ける |
...
- 必殺技で書き換えられる範囲は旧作どおり `turn − 2` 〜 `turn + 2`。今のキャラはリターンが 1 回までなので過去の 2 ターンを書き換えても効果はないが、将来リターン 2 回以上のキャラを作る余地として残す（2026-09-26）。
- 挑発の上乗せは属性変化で消えない（旧作は実装の都合で消えていた。直観に合わせて変更: 2026-09-26）。
- 敵の必殺技はプレイヤーの今の HP と同じダメージにし、必ず負けるようにする（旧作は `攻撃力 × 100` で、HP や防御が高いと耐えられた。最終ターンまでに倒すというルールを確実にするため変更: 2026-09-26）。
